...
/**
 * Generate an email-safe HTML button for a URL
 */
function createButtonHtml(url: string, label: string = "Learn More"): string {
  return `<table border="0" cellpadding="0" cellspacing="0" role="presentation" style="margin:16px 0;"><tr><td align="center" bgcolor="#ffb800" style="border-radius:6px;"><a href="${url}" target="_blank" style="display:inline-block;padding:12px 24px;font-family:'IBM Plex Sans',Arial,sans-serif;font-size:14px;font-weight:600;color:#000;text-decoration:none;">${label}</a></td></tr></table>`;
}

/**
 * Convert URLs in text to styled HTML buttons
 * Handles both plain URLs and markdown-style links [label](url)
 */
function convertUrlsToButtons(text: string): string {
  // First, handle markdown-style links: [label](url)
  const markdownLinkRegex = /\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g;
  let result = text.replace(markdownLinkRegex, (_match, label, url) => {
    return createButtonHtml(url, label);
  });

  // Then, handle plain URLs that aren't already in buttons (not inside href="")
  // Match URLs that are not preceded by href=" or already converted
  const plainUrlRegex = /(?<!href="|>)(https?:\/\/[^\s<>"]+)/g;
  result = result.replace(plainUrlRegex, (url) => {
    return createButtonHtml(url, "Learn More");
  });
...
function formatCoachingToHtml(coachingText: string): string {
  ...
  // Convert URLs to buttons before processing lines
  cleaned = convertUrlsToButtons(cleaned);

  const lines = cleaned.split("\n");

  const coachingHtml = lines
    .map((l) => {
      // Skip wrapping button tables in <p> tags - they're already block elements
