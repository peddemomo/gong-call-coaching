...
const valuePointSchema = z.object({
  listen_for: z.string().min(1, "Listen for is required"),
  insight_text: z.string().min(1, "Insight text is required"),
});
...
// GET /strategies/:strategyId/products - List products for a strategy (with value_points)
router.get("/:strategyId/products", async (req: Request, res: Response) => {
  try {
    const { strategyId } = req.params;
...
// POST /strategies/:strategyId/products - Create product with value points
router.post("/:strategyId/products", async (req: Request, res: Response) => {
  try {
    const { strategyId } = req.params;
    ...
    try {
      ...
      for (let i = 0; i < (value_points?.length ?? 0); i++) {
        const vp = value_points![i];
        await client.query(
...
// PATCH /strategies/:strategyId/products/:productId - Update product (and optionally replace value_points)
router.patch("/:strategyId/products/:productId", async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      if (value_points !== undefined) {
        ...
        for (let i = 0; i < value_points.length; i++) {
          const vp = value_points[i];
          await client.query(
            `INSERT INTO public.product_value_points (product_id, listen_for, insight_text, sort_order, created_at)
             VALUES ($1, $2, $3, $4, NOW())`,
...
// DELETE /strategies/:strategyId/products/:productId - Delete product (cascade deletes value_points)
router.delete("/:strategyId/products/:productId", async (req: Request, res: Response) => {
  try {
    const { strategyId, productId } = req.params;
