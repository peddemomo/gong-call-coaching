/**
 * Global Products router - mounted at /products
 * Full CRUD for products (not scoped to a strategy).
 */
...
// GET / - List all products (with value points)
  ...
  try {
    ...
    try {
      ...
      const productResult = await client.query(
        `INSERT INTO public.products (title, description, created_at)
         VALUES ($1, $2, NOW())
         RETURNING *`,
...
router.patch("/:productId", async (req: Request, res: Response) => {
  try {
    ...
    const productCheck = await pool.query(
      "SELECT id, title, description FROM public.products WHERE id = $1",
      [productId]
    );
